    ...
    if (codec && codec->pix_fmts) {
        ...
        if (*p == AV_PIX_FMT_NONE) {
            if (target != AV_PIX_FMT_NONE)
                if (loglevel == 2) 
                	LOGI(                       "Incompatible pixel format '%s' for codec '%s', auto-selecting format '%s'\n",
    ...
    if (codec && codec->sample_fmts) {
        ...
        if (*p == -1) {
            if((codec->capabilities & AV_CODEC_CAP_LOSSLESS) && av_get_sample_fmt_name(st->codecpar->format) > av_get_sample_fmt_name(codec->sample_fmts[0]))
                if (loglevel > 0) 
                	LOGI("Conversion will not be lossless.\n");
            if(av_get_sample_fmt_name(st->codecpar->format))
            if (loglevel == 2) 
            	LOGI(                   "Incompatible sample format '%s' for codec '%s', auto-selecting format '%s'\n",
...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
        if (loglevel > 0) 
        	LOGI("Only video and audio filters supported "
               "currently.\n");
        exit_program(4005);
    }
    ...
    if (in->name) {
        ...
        if (file_idx < 0 || file_idx >= nb_input_files) {
            if (loglevel > 0) 
            	LOGI("Invalid file index %d in filtergraph description %s.\n",
        ...
        if (!st) {
            if (loglevel > 0) 
            	LOGI("Stream specifier '%s' in filtergraph description %s "
                   "matches no streams.\n", p, fg->graph_desc);
            exit_program(4007);
        }
        ...
        if (i == nb_input_streams) {
            if (loglevel > 0) 
            	LOGI("Cannot find a matching stream for "
                       ...
                       const char *filter_name)
{
    ...
    if (!trim) {
        if (loglevel > 0) 
        	LOGI("%s filter not present, cannot limit "
               "recording time.\n", name);
    ...
    if (ret < 0) {
        if (loglevel > 0) 
        	LOGI("Error configuring the %s filter", name);
        return ret;
    }
...
int configure_output_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    if (!ofilter->ost) {
        if (loglevel > 0) 
        	LOGI("Filter %s has an unconnected output\n", ofilter->name);
        exit_program(4012);
    }
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (ist->dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
        if (loglevel > 0) 
        	LOGI("Cannot connect video filter to audio input\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (ist->dec_ctx->codec_type != AVMEDIA_TYPE_AUDIO) {
        if (loglevel > 0) 
        	LOGI("Cannot connect audio filter to non audio input\n");
...
//     }
    ...
    if (audio_volume != 256) {
        ...
        if (loglevel == 2) 
        	LOGI("-vol has been deprecated. Use the volume "
                                  ...
                                  AVFilterInOut *in)
{
    ...
    if (!ifilter->ist->dec) {
        if (loglevel > 0) 
        	LOGI(               "No decoder for stream #%d:%d, filtering impossible\n",
...
int configure_filtergraph(FilterGraph *fg)
{
    ...
    if (simple && (!inputs || inputs->next || !outputs || outputs->next)) {
        ...
        if (loglevel > 0) 
        	LOGI("Simple filtergraph '%s' was expected "
    ...
    for (i = 0; i < fg->nb_outputs; i++) {
        ...
        if (!ost->enc) {
            ...
            if (loglevel > 0) 
            	LOGI("Encoder (codec %s) not found for output stream #%d:%d\n",
